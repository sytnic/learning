...
Две таблицы: лоты и здания.  

```sql
-- Сеанс 1-го пользователя
...
Предоположим, в это же время второй пользователь удаляет и лот, и здание.

```sql
-- Сеанс 2-го пользователя
...
FROM Lot
WHERE Id = 14;
-- Теперь удалено, нет ответа.

```

Теперь то же самое, но с использованием транзакций.

```sql
-- Вернём данные в таблицы
INSERT INTO Lot
VALUES ('Lot 14', 14);
...
```sql
-- Сеанс 1-го пользователя. Начало транзакции чтения.
BEGIN TRANSACTION;
SELECT *
FROM Building
WHERE Id = 100;
...
```sql
-- Сеанс 2-го пользователя. Начало транзакции записи.
BEGIN TRANSACTION;
DELETE FROM Building WHERE Id = 100;
DELETE FROM Lot WHERE Id = 14;
```
...
```sql
-- Сеанс 1-го пользователя. Данные не удалены
SELECT *
FROM Lot
WHERE Id = 14;
-- Завершить транзакцию чтения
COMMIT;
```
`COMMIT`, `END` и `END TRANSACTION` одинаково завершают транзакцию.    
`ROLLBACK` отменяет транзакцию.

```sql
-- Сеанс 2-го пользователя. Завершение транзакции удаления.
COMMIT;
```

```sql
-- Сеанс 1-го пользователя. Данные исчезли.
SELECT *
FROM Building
WHERE Id = 100;
