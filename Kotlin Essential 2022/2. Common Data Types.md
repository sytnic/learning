...
fun main() {
    ...
    // Вызывает ошибку, т.к. не предназнчен для конкатенации и пытается математически складывать Char, одиночные символы
  ...
  text
    ...
    val someString3 = "  "
    println(someString3.isNotBlank())  // false, не засчитывает пробелы, "является не-пустым ? - ложь, пустой"
